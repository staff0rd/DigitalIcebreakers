...
Use nested `describe` blocks to clearly express context and avoid duplication in test names:

```typescript
// ❌ WRONG - Test name includes context that should be in describe block
...
// ✅ CORRECT - Context in describe blocks, assertions in it blocks
describe("PaymentProcessor", () => {
  describe("when insufficient funds", () => {
    it("should decline payment", () => {
      // ...
    });
  });
...
- Nest describes to build up complex scenarios incrementally

```typescript
// ✅ CORRECT - Clean test structure without duplication
describe("ScoreDisplay", () => {
  ...
  describe("when left team scores", () => {
    it("should display 1-0", () => {
      // Test left scoring from initial state
    });
  });
...
### Testing Components with Canvas/WebGL

When components use canvas-based rendering (PIXI.js, Three.js, etc.), visual output cannot be tested with React Testing Library. Instead:

1. **Extract testable UI to standard React components** - Move scores, status text, controls to MUI/HTML components
2. **Mock canvas libraries in tests** to avoid errors:
   ```typescript
   vi.mock("pixi.js", () => ({
     Application: vi.fn(),
     Graphics: vi.fn(() => ({
   ...
   ```
3. **Test the behavior, not the canvas** - Focus on what users experience, not pixel-perfect rendering
4. **Delete tests that only verify internal state** - If you can't test the user-visible behavior, the test has no value
...
When migrating from Redux to Jotai or other state management solutions:

1. **Focus on testing behavior, not state changes** - Tests that check Redux actions/reducers are testing implementation details
...
4. **Test structure example:**
   ...
   describe("when player is assigned to blue team", () => {
     it("should display blue control buttons", () => {
       // Render component, simulate team assignment, verify button colors
     });
   });
   ```
