...
const connector = connect(null, { adminMessage });

type PropsFromRedux = ConnectedProps<typeof connector> & SlideshowProps;

export class SlideshowPresenter extends BaseGame<
  PropsFromRedux,
  SlideshowPresenterState
> {
  ...
  componentDidMount() {
    ...
    Reveal.addEventListener("fragmentshown", (event: SlideEvent) =>
      this.reportState()
    );
    Reveal.addEventListener("fragmenthidden", (event: SlideEvent) =>
      this.reportState()
    );
