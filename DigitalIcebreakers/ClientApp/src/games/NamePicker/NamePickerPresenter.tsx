...
export default () => {
    ...
    const randomizeOrder = (text: AnimatedText) => app?.stage.addChildAt(text, Random.between(0, app?.stage.children.length-1));
    ...
    const draw = (delta: number) => {
        if (app) {
            const alpha = calculateAlpha();
            users.forEach(u => {
                const text = app.stage.children.find(t => t.name === u.id) as AnimatedText;
        ...
        if (app) {
            users.forEach(u => {
                if (!app.stage.children.find(p => p.name === u.id)) {
                    ...
                    console.log(`placing ${u.name} at ${text.position.x},${text.position.y}`)
                }
            });
            app.stage.children.forEach(displayObject => {
